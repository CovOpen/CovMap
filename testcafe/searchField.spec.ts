...
test("find search field", async (t) => {
  ...
  const websiteText = Selector("html").textContent;
  await t.takeScreenshot().click(riskButton).expect(websiteText).contains("Kontaktverhalten der Bevölkerung");

  // close panel for risk description
  var closeRiskButton = Selector("button")
  ...
  await t
    .click(closeRiskButton.nth(1))
    .expect(websiteText)
    .notContains("Kontaktverhalten der Bevölkerung")
    .takeScreenshot();
});
...
test("Hamburger menu", async (t) => {
  const websiteText = Selector("html").textContent;
  // open menu panel
  var openHamburgerMenu = Selector("button")
    .withAttribute("aria-label", "Main Menu")
    .withAttribute("aria-controls", "menu-appbar");
  var closeHamburgerMenu = Selector("button")
