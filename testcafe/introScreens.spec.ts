...
fixture `Intro screens test`
  .page `http://test.covmap.de`;
...
test('Finish intro screens', async t => {
  ...
  await t
    .expect(websiteText).contains(titleForIntro1)
    .click(nextButton);

  await t
    .expect(websiteText).contains(titleForIntro2)
    .click(nextButton);

  await t
    .expect(websiteText).contains(titleForIntro3)
    .click(nextButton);

  await t
    .expect(websiteText).contains(titleTextForPostalCode)
    .pressKey("1 2 3 4 5 enter");

  await t
    .expect(websiteText).notContains(titleTextForPostalCode)
});
