...
const viewportSubsetFiler = createBlacklistFilter("viewport", ["transitionInterpolator"]);

const reviveCurrentLayerGroup = createTransform(
  (inboundState: any, key: string | number | symbol, state: any) => {
    return "currentLayerGroup" === key ? inboundState.id : inboundState;
  },
  ...
  return persistReducer(
    {
      ...
      storage,
      whitelist,
      transforms: [viewportSubsetFiler, reviveCurrentLayerGroup],
    },
    reducer,
  );
}
